...
1. **Small / medium / standalone-degraded trivial → a new issue, milestone-attached or backlog per `docs/heal-routing.md`'s reconciliation.** Open it via `gh issue create` (the same primitive as the write-up's redo one-liner), carrying the finding's `description` + `symbol` and its single `grounding` ref so the issue starts hard-grounded — attached to the active milestone, or opened as a backlog issue when none is, per that reconciliation. A same-milestone fix is **not** re-coherence-reviewed (`docs/heal-routing.md` §"Same-milestone (or backlog) fixes are not re-reviewed"); a backlog issue carries no milestone loop to re-enter in the first place.
