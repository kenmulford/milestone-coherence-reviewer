...
This skill is the **orchestrator**. The review engine (`agents/coherence-reviewer.md`) is **read-only** — it returns findings **and** proposals; this skill performs the heal. The heal is healing, never gating: it fixes what it reasonably can and reports the rest, and **the change it reviewed merges either way** (`BRIEF.md` §"It heals, it doesn't gate" l.42-52). It opens follow-up issues / a follow-up milestone for the fixes it cannot apply; it never blocks, gates, or touches the merge, and it never writes to or force-pushes the protected branch (`BRIEF.md` l.50, l.125-126).
...
1. **Assemble the review context (once)** — its five parts (`docs/analyze-once.md` Step 1):
   ...
   | the recalled prior write-ups (advisory) | prior write-ups matched to the diff's touched paths, handed to the engine as advisory context — mechanics in **`docs/recall.md`** (item 2 below); source = the orchestrator |

2. **Read recall once (advisory) — the fifth context part.** After item 1's diff makes the touched paths known and **before** the engine dispatch, read back prior write-ups **once**, orchestrator-side, via `scripts/memory-mirror.<sh|ps1> --recall` (host as Step 1). The engine runs no recall itself (`agents/coherence-reviewer.md` §"Recalled prior write-ups (advisory only)"). All gather mechanics — the invocation form, the parse-once of #69's TAB record stream, the verbatim `SECTION-BEGIN … SECTION-END` hand-off, and the exit-0 empty-state vs. nonzero-exit failure split — live in **`docs/recall.md`** §"Step 2 — gather the recall read-back (once)", the single source.
...
5. **Render the recall status note — never silence.** Keyed off the `entries=<N>` count from Step 2 item 2's single SUMMARY parse: N > 0 references the matched prior write-up(s); N = 0 / `NONE` / recall unavailable renders the explicit **"no related prior write-ups found"** note. This is a **procedural status note** (like the D17 nudge at Step 1.4), exempt from the engine-`FINDINGS`/`PROPOSALS`-fields-only render rule (`docs/write-up.md` §"What the write-up renders from" — the procedural-status-note exception list) and part of the inline content the mirrors (item 6) copy. The two-branch render contract lives in **`docs/recall.md`** §"Step 4 — render the recall status note (never silence)", the single source.
...
### Step 5 — Route each fix by DRIFT SIZE (enact #6, standalone mode)

For each finding, route on its `severity` field **and nothing else** — not run length, not finding count, not token budget (`docs/heal-routing.md` §"The single routing key"; `BRIEF.md` l.52, l.109). This run is **standalone mode**, so the `drift-trivial` route degrades (there is no driver build loop to re-dispatch the implementer into). Each route's slice is the minimal, self-contained #4 slice — never the whole analysis (`docs/analyze-once.md` §"Self-contained or it doesn't ship"). (Convention **proposals** are a separate lane — **not** routed here; they were authored as a config-only PR at Step 3.)

**The route table and the milestone-attachment reconciliation live in `docs/heal-routing.md` alone** — this skill carries no copy of either. See its route table (`docs/heal-routing.md` §"The route table (mutually exclusive, total over the severity enum)") for every `severity` value's destination, and its `docs/heal-routing.md` §"Milestone attachment reconciliation (not a hard-coded assumption)" for how each new-issue route decides whether it attaches to an active milestone or opens as a backlog issue.

1. **Small / medium / standalone-degraded trivial → a new issue, milestone-attached or backlog per `docs/heal-routing.md`'s reconciliation.** Open it via `gh issue create` (the same primitive as the write-up's redo one-liner), carrying the finding's `description` + `symbol` and its single `grounding` ref so the issue starts hard-grounded. **The body is shaped to the consumer repo's issue template — the resolution ladder, the emission modes, the stock mapping, and their degradation live in `docs/issue-templates.md` alone; this skill carries no copy.** A same-milestone fix or backlog issue is **not** re-coherence-reviewed (`docs/heal-routing.md` §"Same-milestone (or backlog) fixes are not re-reviewed").
...
3. **The deferred boundary — state it, do NOT cross it.** After the feeder creates the follow-up milestone, **standalone v1 CANNOT auto-run `milestone-driver`** — the **unattended** `feeder → driver` auto-handoff (no human between milestones) still does not exist and remains a separate companion change (`docs/heal-routing.md` §"The deferred boundary"; `BRIEF.md` l.83, l.141). `milestone-feeder`#148 shipped a narrower, human-gated create-time offer (`autoHandoff`, default `prompt`) — not this unattended cycle. **State this in the write-up** so the reader knows the follow-up milestone is created but **not yet auto-built into that unattended cycle** — a human runs (or confirms) the driver on it. End cleanly; do not present the driver as if it will pick the milestone up with no human involved.

4. **`FINDINGS: none` → route nothing.** Open no issue, hand nothing to the feeder, re-dispatch no one (`docs/heal-routing.md` §"Empty / zero-findings"). The write-up already reported the clean fit at Step 4.
...
- **Never a merge gate.** The review is post-build and read-only on the merge: it heals what it reasonably can and reports the rest, and the change it reviewed merges regardless of any finding, drift size, or route — no hard stop and no merge-blocking verdict, for **every** drift size and **every** path (`docs/heal-routing.md` §"Never a gate"; `BRIEF.md` l.42-44, l.50, l.124-125).
- **Never touches the protected branch.** It writes no application code, creates no application-code branch, force-pushes nothing, and never writes to or force-pushes the protected branch. Its writes are follow-up issues, a feeder brief, comment/memory mirrors, and — for a proposal — a config-only PR to `integrationBranch` (a `chore/propose-<slug>` branch + a `.project/conventions.md` entry), never the protected branch (`BRIEF.md` l.126).
...
- **Analyze once, distribute slices.** The review context is assembled once (including the advisory recall read-back — gathered once at Step 2, never re-read) and the engine dispatched once per review call; downstream routes get minimal self-contained slices — no re-gather, re-analyze, or re-resolve (`docs/analyze-once.md`; `BRIEF.md` l.35-40, l.114).
