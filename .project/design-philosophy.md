...
> **Read-only engine ↔ orchestrator split.** The `coherence-reviewer` agent RETURNS findings **and** proposals — it writes no files, opens no issues, opens no PR, runs no `gh`, never edits the repo, and never touches the protected branch (the orchestrator performs the heal and opens any config-only PR). The `review` skill (the orchestrator) performs the heal. Coherence reads the suite's shared mechanics from `.milestone-config/driver.json` **in place** (never duplicating them) and reads `.project/` sections via the installed driver's `read-doc-section` primitive (wired, not reimplemented). (BRIEF.md §"Recorded decisions" l.113, §"Integration" l.89-90; agents/coherence-reviewer.md "Read-only"; skills/review/SKILL.md "Non-negotiables".)
...
> Built via the `feeder → driver` dogfood loop with `/code-review` per PR. The cross-platform script twins are verified **byte-for-byte identical** between the bash and PowerShell hosts (`resolve-config`, `memory-mirror`). The "verified" bar is documented-contract conformance, bash/pwsh parity, and the CI gates — not an application unit-test suite, because this is a markdown + shell plugin, not compiled code. The full bar, including the CI gates and the golden-case fixtures that self-test the size-budgets checker, is enumerated once at `.project/conventions.md#Test patterns`. (CHANGELOG.md v0.1.0 table + "Post-run audit trail"; docs/resolution.md §"Section-body trailing blank lines"; .github/workflows/ci.yml.)
