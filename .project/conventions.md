...
> **SemVer.** The single source of version truth is `.claude-plugin/plugin.json` (`version`); `marketplace.json` carries **no** `version` field — Claude Code resolves `plugin.json` first, and setting both silently masks the marketplace value. Milestones are named as versions (e.g. `v0.1.0`, `v0.1.1`) so the driver can derive the bump target. (BRIEF.md §"Plugin packaging" l.102-104; .claude-plugin/plugin.json `version` 0.1.0; .milestone-config/feeder.json `versioning: semver`.)
