...
- **Embedded in the driver (v2, shipped as a companion change in `milestone-driver`).** The full loop: dispatched at a new `solve-issue` step ~6.2 (after `/code-review` converges, before commit), read-only, returning findings + heal-route + the write-up. The **driver orchestrates the heal** (re-dispatch implementer for small / open current-milestone issues for medium / hand large adjustments to `milestone-feeder`, which builds the follow-up milestone) — mirroring how triage/design-reviewer/implementer stay read-only while the orchestrator acts. Wired via a default-filled `coherenceReviewAgent` profile key, exactly like `triageAgent` / `designReviewAgent` / `implementerAgent`. **Shipped** — [kenmulford/milestone-driver#231](https://github.com/kenmulford/milestone-driver/issues/231), released in `milestone-driver` v1.13.0 (2026-06-24): coherence review runs before the final `/code-review` in `solve-issue`/`solve-milestone`, gated on the reviewer being present + configured, silently skipped otherwise, never gating.

This repo ships the reviewer; the step-6.2 dispatch, the `coherenceReviewAgent` key, and the heal-orchestration now ship in `milestone-driver` (above — `kenmulford/milestone-driver#231`, v1.13.0).

**Cross-plugin capability, partially shipped:** the fully-automated `feeder → driver` handoff — the driver **auto-running** a feeder-created follow-up milestone after the active milestone completes, with **no human in between** — **still does not exist**. What shipped ([kenmulford/milestone-feeder#148](https://github.com/kenmulford/milestone-feeder/issues/148), `milestone-feeder` v0.5.0) is narrower: `create` now **offers** to hand a clean run straight to the driver, gated by `feeder.json#autoHandoff` (`prompt` default — a human must say yes; `auto` — no prompt, if explicitly configured; `off` — never offered). Standalone v1 can create the follow-up milestone via the feeder but still **cannot itself auto-run the driver**; the unattended, no-human-by-default cycle this section originally flagged remains unbuilt.
...
- **Heals, never gates.** No merge-blocking. Routes the fix by **drift size only**: trivial → inline; small/medium → current-milestone issue; large → handed to `milestone-feeder`, which builds the follow-up milestone (the driver auto-running that milestone after the active one completes, no human in between, remains the target design — not yet built). **Run length is a driver-side context concern (compaction / subagent isolation), not a coherence routing signal.** The unattended `feeder → driver` auto-handoff is still not existing behavior; `milestone-feeder`#148 shipped a narrower, human-gated create-time offer (`autoHandoff`, default `prompt`), not this full cycle.
...
Running it on a built change produces a **grounded, legible report** — what fits, what doesn't, why, with citations and copy-paste `gh` one-liners — and routes fixes by drift size (trivial inline / small-medium current-milestone issue / large fed to the feeder, which builds the follow-up milestone; the driver auto-running that milestone after the active one completes, no human in between, remains the target design) without ever blocking the merge. Standalone works on its own; the driver-embedded path **shipped** as its companion milestone (`kenmulford/milestone-driver#231`, v1.13.0); the fully-automated `feeder → driver` auto-handoff remains unbuilt — `kenmulford/milestone-feeder#148` shipped only a narrower, human-gated create-time offer (`autoHandoff`, default `prompt`).
