...
| write the entry | append to `.project/conventions.md`, mirroring the existing shape (`.project/conventions.md` header comment): a stable `## <heading>` + a `> <rule>` blockquote + the `exemplar`, as either `path (anchor)` or `path:line`; optionally a row in the "Canonical exemplars" table. Both exemplar forms are defined at `milestone-driver/skills/citation-format.md § The four forms`; **prefer the anchor form** here, since the entry outlives the line its exemplar sits on today. A preference, not a requirement; `path:line` stays fully valid, and a citation is one form or the other, never a hybrid, so `path (anchor)` carries no line number. This slot takes a repo file ref, so a heading target is cited here in the anchor or line form. When `disagree: yes`, the `rule` recommends the grounded winner and the entry notes the `diverging` sites |
